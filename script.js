...
class App {
  ...
  constructor() {
    ...
    //* Получание данных из localeStorage
  ...
  _getPosition() {
    //* Проверяем, поддерживает ли браузер navigation.geolocation
  ...
  //* Получаем данные своей геолокации
  _loadMap(position) {
    ...
    // console.log(
    //   `https://www.google.ru/maps/@${latitude},${longitude},11z?entry=ttu`
    // );
    const coords = [latitude, longitude];
    //* Добавляем карту на страницу при помощи библиотеки
    ...
    //* Обработка клика на картке
    this.#map.on('click', this._showForm.bind(this));

    //* Отображение тренировок из locale storage на карте
    this.#workouts.forEach(el => this._displayWorkout(el));
  }
  ...
  //* Отображение определенного инпута взависимости от вида кардио
  _toggleClimbField() {
  ...
  _getLocaleStorageData() {
    ...
    if (!data) return;
    this.#workouts = data;
    this.#workouts.forEach(el => this._displayWorkoutOnSidebar(el));
  }
